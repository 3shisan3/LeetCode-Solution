...
public:
    int idealArrays(int n, int maxValue)
    {
        ...
        function<int(int, int, int)> dp = [&](int index, int lastValue, int curNum) -> int
        {
            if (index == n - 1 || curNum == 1)
            {
                return curNum;
            }
            ...
            {
                res += dp(index + 1, curValue, maxValue / curValue);
                res %= MOD;
        ...
        return dp(0, 1, maxValue);
    }
};

// 递归超时了，但官解也很死板（没感受到意义，还是先求质数集合，再就是丢球到盒问题）（不如直接丢球）
/* 
